...
const BASE=[
 {id:'E-101',remito:'R-10482',material:'Kit de curación avanzada',cantidad:'1',sanatorio:'Sanatorio Parque',paciente:'María Gómez',medico:'Dr. Federico López',assignedTo:'SOL',state:'assigned',fecha:'2026-08-14',note:'Entregar en recepción de quirófano.'},
 ...
 {id:'E-105',remito:'R-10479',material:'Apósito antimicrobiano',cantidad:'3',sanatorio:'Sanatorio Parque',paciente:'Roberto Silva',medico:'Dr. Federico López',assignedTo:'DIEGO',state:'done',fecha:'2026-08-12',exitBy:'Diego Demo',deliveredBy:'Diego Demo',receivedBy:'Mariana Torres',sector:'Quirófano 2',note:'Remito firmado por coordinación.',deliveredAt:'2026-08-12T15:45:00'}
];
let session=null,records=[],current=null,lastSync=null,detailReturn='myDeliveries';
...
async function init(){
 ...
 $('activate').onclick=activate;$('sync').onclick=sync;$('logout').onclick=logout;$('confirmDelivery').onclick=confirmDelivery;$('remitoPhoto').onchange=preview;$('recordSearch').oninput=renderRecords;$('stateFilter').onchange=renderRecords;$('userFilter').onchange=renderRecords;$('otherSearch').oninput=renderOtherMatches;
...
function taskCard(r,admin=false,source='myDeliveries',viewOnly=false){return `<div class="task-card ${viewOnly?'search-result':''}"><div class="task-top"><div><div class="task-title">${r.remito} · ${r.material}</div><div class="task-meta">${r.sanatorio}<br>${r.paciente} · ${r.medico}${admin?`<br><strong>Asignado a: ${userName(r.assignedTo)}</strong>`:''}</div></div><span class="pill ${r.state==='out'?'out':r.state==='done'?'done':''}">${stateName(r.state)}</span></div>${r.note?`<div class="note-box"><strong>Nota:</strong> ${r.note}</div>`:''}<div class="task-actions"><button class="secondary" onclick="openDetail('${r.id}','${source}')">Ver detalle</button>${!viewOnly?(r.state==='assigned'?`<button class="primary" onclick="markExit('${r.id}')">Marcar retirado</button>`:r.state==='out'?`<button class="primary" onclick="openDelivery('${r.id}')">Completar entrega</button>`:''):''}</div></div>`}
function renderAssigned(){const a=ownActive();$('assignedList').innerHTML=a.length?a.map(r=>taskCard(r,false,'assigned')).join(''):`<div class="empty">No tenés entregas pendientes asignadas.</div>`}
function renderMyDeliveries(){const a=records.filter(r=>r.assignedTo===session.userId);$('myDeliveriesList').innerHTML=a.length?a.map(r=>taskCard(r,false,'myDeliveries')).join(''):'<div class="empty">No hay entregas.</div>'}
function detail(r){return `<div class="detail-grid"><div class="detail-row"><small>N.º de remito</small><strong>${r.remito}</strong></div><div class="detail-row"><small>Material</small><strong>${r.material} · ${r.cantidad} unidad(es)</strong></div><div class="detail-row"><small>Sanatorio</small><strong>${r.sanatorio}</strong></div><div class="detail-row"><small>Paciente</small><strong>${r.paciente}</strong></div><div class="detail-row"><small>Médico</small><strong>${r.medico}</strong></div><div class="detail-row"><small>Asignado a</small><strong>${userName(r.assignedTo)}</strong></div><div class="detail-row"><small>Estado</small><strong>${stateName(r.state)}</strong></div>${r.exitBy?`<div class="detail-row"><small>Retirado por</small><strong>${r.exitBy} · ${fmt(r.exitAt)}</strong></div>`:''}${r.deliveredBy?`<div class="detail-row"><small>Entregado por</small><strong>${r.deliveredBy} · ${fmt(r.deliveredAt)}</strong></div>`:''}${r.receivedBy?`<div class="detail-row"><small>Recibido por</small><strong>${r.receivedBy} · ${r.sector||''}</strong></div>`:''}${r.note?`<div class="detail-row"><small>Nota</small><strong>${r.note}</strong></div>`:''}</div>`}
function openDetail(id,from){current=records.find(r=>r.id===id);detailReturn=from||detailReturn||'myDeliveries';const actionAllowed=detailReturn!=='records';$('detailBody').innerHTML=detail(current)+(actionAllowed?`<div style="margin-top:14px">${current.state==='assigned'?`<button class="primary" onclick="markExit('${id}')">MARCAR COMO RETIRADO</button>`:current.state==='out'?`<button class="primary" onclick="openDelivery('${id}')">COMPLETAR ENTREGA</button>`:''}</div>`:'');show('detail')}
...
function openDelivery(id){current=records.find(r=>r.id===id);$('deliveryReadOnly').innerHTML=detail(current);$('receivedBy').value='';$('sector').value='';$('deliveryNote').value=current.note||'';$('remitoPhoto').value='';$('photoPreview').classList.add('hidden');show('delivery')}
...
async function confirmDelivery(){if(!current)return;const rec=$('receivedBy').value.trim(),sec=$('sector').value.trim(),f=$('remitoPhoto').files[0];if(!rec||!sec){alert('Completá Recibido por y Sector.');return}if(!f){alert('La foto del remito es obligatoria.');return}const rd=new FileReader();rd.onload=async()=>{current.state='done';current.receivedBy=rec;current.sector=sec;current.note=$('deliveryNote').value.trim();current.remitoPhoto=rd.result;current.deliveredBy=session.name;current.deliveredAt=new Date().toISOString();await save();alert('Entrega registrada.');show('myDeliveries')};rd.readAsDataURL(f)}
async function save(){await idbSet(STORES.kv,'recordsV4',records);renderAssigned();renderMyDeliveries()}
function prepareOther(){$('otherSearch').value='';$('otherPreview').innerHTML='<div class="muted" style="font-size:12px">Buscá por número de remito o paciente.</div>';renderOtherMatches()}
function renderOtherMatches(){if(!session?.superAdmin)return;const q=$('otherSearch').value.trim().toLowerCase();const arr=records.filter(r=>r.assignedTo!==session.userId&&r.state!=='done'&&(!q||r.remito.toLowerCase().includes(q)||r.paciente.toLowerCase().includes(q)));$('otherMatches').innerHTML=q?(arr.length?arr.map(r=>`<div class="search-hit"><strong>${r.remito} · ${r.paciente}</strong><div class="task-meta">${r.sanatorio}<br>${r.material}<br>Asignado a: ${userName(r.assignedTo)}</div><button class="secondary" onclick="selectOther('${r.id}')">Seleccionar</button></div>`).join(''):'<div class="empty">No encontramos una entrega pendiente.</div>'):''}
function selectOther(id){const r=records.find(x=>x.id===id);current=r;$('otherPreview').innerHTML=detail(r)+`<div style="margin-top:14px">${r.state==='assigned'?`<button class="primary" onclick="markExit('${r.id}')">MARCAR COMO RETIRADO</button>`:`<button class="primary" onclick="openDelivery('${r.id}')">COMPLETAR ENTREGA</button>`}</div>`;$('otherMatches').innerHTML=''}
